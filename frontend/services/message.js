...
export const addMessage = async ({ content, name, email, phone }) => {
  try {
    const { data } = await axios.post(
      `${process.env.NEXT_PUBLIC_PROXY}/api/custom-orders`,
      {
        content,
        name,
        email,
...
export const approveCustomOrder = async ({ id, meetingDate }) => {
  try {
    ...
    const data = await axios.put(
      `${process.env.NEXT_PUBLIC_PROXY}/api/custom-orders/approve/${id}`,
      { meetingDate }
    );
...
export const rejectCustomOrder = async ({ id }) => {
  try {
    const data = await axios.put(
      `${process.env.NEXT_PUBLIC_PROXY}/api/custom-orders/reject/${id}`
    );
...
export const rescheduleCustomOrder = async ({ id, meetingDate }) => {
  try {
    const data = await axios.put(
      `${process.env.NEXT_PUBLIC_PROXY}/api/custom-orders/reschedule/${id}`,
      { meetingDate }
    );
...
export const getMeetings = async ({ d }) => {
  try {
    const data = await axios.get(
      `${process.env.NEXT_PUBLIC_PROXY}/api/custom-orders/meetings`,
      { params: { d } }
    );
